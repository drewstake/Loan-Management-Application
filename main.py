...
def refresh_customers(search_term=""):
    ...
    for r in cur.fetchall():
        cust_tree.insert("", tk.END, values=r)
    conn.close()
...
def refresh_loans(search_term=""):
    ...
    if search_term.strip():
        ...
        """)
    for r in cur.fetchall():
        loan_tree.insert("", tk.END, values=r)
    conn.close()
...
cust_name, cust_phone, cust_search = (
...
ttk.Label(form2, text="Amount:").grid(row=0, column=0)
ttk.Entry(form2, textvariable=loan_amount).grid(row=0, column=1)
ttk.Label(form2, text="Rate %:").grid(row=1, column=0)
ttk.Entry(form2, textvariable=loan_rate).grid(row=1, column=1)
ttk.Label(form2, text="Type:").grid(row=2, column=0)
ttk.Entry(form2, textvariable=loan_type).grid(row=2, column=1)
ttk.Label(form2, text="Cust ID:").grid(row=3, column=0)
ttk.Entry(form2, textvariable=loan_cust_id).grid(row=3, column=1)
